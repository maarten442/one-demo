...
def extract_relevant_sections(question: str, manual_content: str, max_context_length: int = 8000) -> str:
    """Extract relevant sections from the manual using intelligent section-based retrieval"""
    
    # Use the smart section parser
    relevant_content = get_relevant_sections_smart(question, manual_content)
    
    # Limit to max length if needed
    if len(relevant_content) > max_context_length:
        relevant_content = relevant_content[:max_context_length]
    
    return relevant_content if relevant_content else manual_content[:max_context_length]
...
4. Vermeld belangrijke waarschuwingen met ⚠️
